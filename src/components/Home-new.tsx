...
const Home: React.FC<HomeProps> = ({ onLogin, onViewNews, onJoin }) => {
  ...
  // Local state for the modal toggle
      ...
      <nav className="border-b border-kala-800 bg-kala-900/80 backdrop-blur sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-4">
            <button
              onClick={onViewNews}
              className="text-sm font-medium text-kala-300 hover:text-white flex items-center gap-2 px-4 py-2 rounded hover:bg-kala-800 transition-colors"
            >
      ...
      <footer className="bg-kala-900 border-t border-kala-800 py-12">
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-4">
            ...
            <a href="https://www.instagram.com/kalakrut" target="_blank" rel="noreferrer" className="p-2 bg-kala-800 rounded-full text-kala-400 hover:text-white hover:bg-kala-700 transition-colors">
              <Instagram className="w-4 h-4" />
            </a>
            <a href="https://www.facebook.com/kalakrut" target="_blank" rel="noreferrer" className="p-2 bg-kala-800 rounded-full text-kala-400 hover:text-white hover:bg-kala-700 transition-colors">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-in fade-in">
          <div className="bg-kala-900 border border-kala-700 rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl relative">
                ...
                <div className="flex justify-center mt-6">
                  <div className="bg-kala-800 p-1 rounded-lg inline-flex relative">
                    <button
                      ...
                      className={`relative z-10 px-4 py-2 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${loginMode === 'demo' ? 'bg-kala-secondary text-kala-900 shadow' : 'text-kala-400 hover:text-white'}`}
                    >
                      <PlayCircle className="w-3 h-3" /> Demo Mode
                    </button>
                    <button
                      onClick={() => { setLoginMode('live'); setEmail(''); setPassword(''); }}
                      className={`relative z-10 px-4 py-2 rounded-md text-xs font-bold transition-all flex items-center gap-2 ${loginMode === 'live' ? 'bg-green-500 text-white shadow' : 'text-kala-400 hover:text-white'}`}
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 p-8 border-t border-kala-800 bg-kala-800/20">
              {/* Web3 Login */}
              <div className={`flex flex-col items-center p-6 bg-kala-800/50 rounded-xl border border-kala-700 transition-colors group ${loginMode === 'live' ? 'opacity-60' : 'hover:border-purple-500'}`}>
                <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 transition-colors ${loginMode === 'live' ? 'bg-kala-700 text-kala-500' : 'bg-purple-500/10 text-purple-400 group-hover:bg-purple-500 group-hover:text-white'}`}>
                  <Wallet className="w-8 h-8" />
                </div>
                <h3 className="text-lg font-bold text-white mb-2">Web3 Wallet</h3>
                <p className="text-xs text-kala-400 text-center mb-6 h-10">
                  {loginMode === 'live' ? 'Wallet connect is disabled in Live Mode for security reasons.' : 'Connect MetaMask, Phantom, or WalletConnect.'}
                </p>
                <button
                  onClick={() => handleLoginClick('web3')}
                  disabled={isLoading || loginMode === 'live'}
                  className="w-full py-2 bg-purple-600 text-white font-bold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:bg-kala-700 disabled:text-kala-500 disabled:cursor-not-allowed"
                >
                  {isLoading && loginMode !== 'live' ? <Loader2 className="w-4 h-4 animate-spin" /> : (loginMode === 'live' ? 'Disabled' : 'Connect Wallet')}
                </button>
              </div>
              ...
              <div className="flex flex-col items-center p-6 bg-kala-800/50 rounded-xl border border-kala-700 hover:border-kala-secondary transition-colors group">
                ...
                {loginMode === 'live' ? (
                  ...
                  <div className="w-full space-y-3">
                    <div className="bg-kala-900/50 border border-kala-700 p-3 rounded text-center text-xs text-kala-400">Simulated Environment Active</div>
                    <button
                      onClick={() => handleLoginClick('web2')}
                      disabled={isLoading}
                      className="w-full py-2 bg-kala-secondary hover:bg-cyan-400 text-kala-900 font-bold rounded-lg transition-colors flex items-center justify-center gap-2"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="pt-4 border-t border-kala-800">
                <button
                  ...
                  className="w-full py-3 bg-kala-secondary text-kala-900 font-bold rounded-xl hover:bg-cyan-400 transition-colors shadow-lg"
                >
                  Join the Community
                </button>
              </div>
