import React, { useState, useEffect, Suspense, useTransition } from 'react'; [cite_start]//[span_1](end_span)
...
[span_2](start_span)// Lazy Loaded Components [cite: 8-13]
...
const AppContent: React.FC = () => {
  const [isPending, startTransition] = useTransition(); [cite_start]// Added Transition hook[span_2](end_span)
  ...
  [span_3](start_span)[span_4](start_span)// Use this function for all navigation to avoid suspension errors[span_3](end_span)[span_4](end_span)
  ...
  const handleLogin = async (role: UserRole, method: 'web2' | 'web3', credentials?: any) => {
    ...
    if (isDemoMode) {
       ...
       if (method === 'web2') {
          ...
          const found = findUserByWallet('dummy_address');
    ...
    [span_5](start_span)startTransition(() => { // Wrap login state changes in transition[span_5](end_span)
  ...
  const handleLogout = () => {
    ...
    disconnectWallet();
    navigate('home'); [span_6](start_span)// Use transition navigation[span_6](end_span)
    setIsUserBlocked(false);
    ...
    return (
      <div className="flex items-center gap-2 mr-4 bg-kala-800 rounded-lg p-1">
        ...
        <select value={currentUserRole} onChange={(e) => {
              ...
              [span_7](start_span)startTransition(() => { // Wrap role switch in transition[span_7](end_span)
  ...
  const renderAppContent = () => (
    [span_8](start_span)<div className={isPending ? "opacity-70 transition-opacity pointer-events-none" : "opacity-100 transition-opacity"}> {/* visual indicator during transition[span_8](end_span) */}
      <Suspense fallback={<PageLoader />}>
        {(() => {
          switch (currentView) {
            ...
            case 'contracts': return (currentUserRole === UserRole.ADMIN || currentUserRole === UserRole.DAO_MEMBER) ? <AdminContracts onBlockUser={handleBlockUser} onChat={(name, avatar) => { setChatRecipient({ ...MOCK_ARTIST_PROFILE, name, avatar }); setShowChat(true); }} /> : <div>Access Denied</div>;
            case 'treasury': return (currentUserRole === UserRole.ADMIN || currentUserRole === UserRole.DAO_MEMBER) ? <TreasuryDashboard /> : <div>Access Denied</div>;
            case 'hrd': return (currentUserRole === UserRole.ADMIN || currentUserRole === UserRole.DAO_MEMBER) ? <HRDashboard /> : <div>Access Denied</div>;
